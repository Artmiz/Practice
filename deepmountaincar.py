...
import gym
import gym.spaces
import pdb # import to enable using breakpoints 
...
class DQN :
    ...
    def build_networks(self): # To create primary and target networks using Tensorflow
    ...
    def target_params_replaced(self): # To replace the target network parameters with primary network parameters

            self.sess.run(self.replacing_target_parameters)

    def store_experience(self,obs,a,r,obs_): # To store experiences, that is, tuple of (state, action, reward, new state)
    ...
    def fit(self): # To train Deep-QN  (Dynamic Epsilon greedy)
    ...
    def epsilon_greedy(self,obs): # For a given observation state, which action to take (exploit or explore)
